## Usage [pkgfolders] = get_train_basemodel_folders ( thedir )
##
...
function [trainfolders] = get_train_basemodel_folders(thedir)
  if isdir(thedir) == 0;
...
%!error <is not a directory> get_train_basemodel_folders('');

%!error <is not a directory> get_train_basemodel_folders(program_invocation_name());

%!test
%! test_fname = tempname();
%! mkdir(test_fname);
%! [res] = get_train_basemodel_folders(test_fname);
