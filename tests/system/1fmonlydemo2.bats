...
@test "Runs demo 2, train and predict with --1fmonly flag, est time 5 min" {
    ...
    [ "$status" -eq 0 ]
    run $RUNTRAINING_SH --numiterations 100 --1fmonly "$TEST_TMP_DIR/mito_testaugtrain" "$TEST_TMP_DIR/train_out"
    echo "$status $output" 1>&2
    [ "$status" -eq 0 ]

    run $RUNPREDICTION_SH "$TEST_TMP_DIR/train_out" --models 1fm  ~/cdeep3m/mito_testsample/testset/ "$TEST_TMP_DIR/predictout"
    echo "$status $output" 1>&2
    [ "$status" -eq 0 ] 
